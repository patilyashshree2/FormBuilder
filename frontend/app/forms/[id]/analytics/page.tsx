...
import { fetchAnalytics, getForm, exportCSV } from "../../../api-client";
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800">
      <div className="max-w-7xl mx-auto p-4 lg:p-8">
        ...
        {/* PII Fields Notice */}
        {form.fields.some((f: any) => f.isPII) && (
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="text-lg font-bold text-orange-800 dark:text-orange-200 mb-2">
                  PII Fields Excluded from Analytics
                </h3>
                <p className="text-orange-700 dark:text-orange-300 mb-3">
                  The following fields contain Personally Identifiable Information (PII) and are excluded from analytics for privacy protection:
                </p>
                <ul className="list-disc list-inside space-y-1">
                  {form.fields
                    .filter((f: any) => f.isPII)
                    .map((field: any, index: number) => (
                      <li key={index} className="text-orange-700 dark:text-orange-300 font-medium">
                        {field.label}
                      </li>
                    ))}
                </ul>
                <p className="text-sm text-orange-600 dark:text-orange-400 mt-3">
                  💡 PII data is still collected and available in CSV exports for authorized users.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-gray-900 dark:text-gray-100">{an.completionRate?.toFixed(1) || 0}%</p>
              </div>
        ...
        {/* Response Trends */}
        {an.responseTrends && an.responseTrends.length > 0 && (
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 mb-8">
            <h3 className="text-xl font-bold text-gray-800 dark:text-gray-200 mb-6">Response Trends (Last 7 Days)</h3>
            <div className="flex items-end justify-between h-32 gap-2">
              {an.responseTrends.map((trend: any, index: number) => {
                const maxCount = Math.max(...an.responseTrends.map((t: any) => t.count)) || 1;
                const height = (trend.count / maxCount) * 100;
                return (
                  <div key={index} className="flex-1 flex flex-col items-center">
                    <div 
                      className="w-full bg-blue-500 rounded-t-lg transition-all duration-500"
                      style={{ height: `${height}%`, minHeight: trend.count > 0 ? '4px' : '0px' }}
                    />
        ...
        {/* Most Skipped Fields */}
        {an.skippedFields && an.skippedFields.length > 0 && (
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 mb-8">
            <h3 className="text-xl font-bold text-gray-800 dark:text-gray-200 mb-6">Most Skipped Fields</h3>
            <div className="space-y-4">
              {an.skippedFields
                .filter((field: any) => field.skipRate > 0)
                .sort((a: any, b: any) => b.skipRate - a.skipRate)
                .slice(0, 5)
                .map((field: any, index: number) => (
                  <div key={index} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                    <div>
                      <h4 className="font-medium text-gray-900 dark:text-gray-100">{field.fieldName}</h4>
                      <p className="text-sm text-gray-600 dark:text-gray-400">Skipped {field.skipCount} times</p>
                    </div>
                    <div className="text-right">
                      <div className="text-2xl font-bold text-red-600 dark:text-red-400">{field.skipRate.toFixed(1)}%</div>
